...
const styles = `
  ...
  .glow-wrapper::before {
    ...
    background:radial-gradient(circle,rgba(232,180,168,.25) 0%,transparent 70%);
    top:-100px; right:-100px; pointer-events:none;
  }
  .glow-wrapper::after {
    content:''; position:absolute; width:320px; height:320px; border-radius:50%;
    background:radial-gradient(circle,rgba(200,137,110,.14) 0%,transparent 70%);
  ...
  .glow-card {
    ...
    box-shadow:0 8px 48px rgba(139,77,58,.09);
  }
  .glow-tabs { display:flex; background:#F2EAE4; border-radius:11px; padding:4px; margin-bottom:1.85rem; }
  .glow-tab {
    ...
    color:#9C7B72; cursor:pointer; transition:all .25s ease;
  }
  .glow-tab.active { background:white; color:#8B4D3A; box-shadow:0 2px 10px rgba(139,77,58,.13); }
  .glow-label {
  ...
  .glow-input::placeholder { color:rgba(156,123,114,.5); }
  ...
  .glow-btn-ghost {
    width:100%; padding:11px; border:1px solid rgba(200,137,110,.45); border-radius:11px;
    background:transparent; color:#8B4D3A; font-family:'Jost',sans-serif;
    font-size:13px; font-weight:500; letter-spacing:1px; text-transform:uppercase;
    cursor:pointer; transition:all .2s; margin-top:8px;
  }
  .glow-btn-ghost:hover { background:#FAF0EB; border-color:#C8896E; }
  .glow-divider { display:flex; align-items:center; gap:10px; margin:1.2rem 0; }
  .glow-divider-line { flex:1; height:1px; background:linear-gradient(90deg,transparent,rgba(200,137,110,.22),transparent); }
  .glow-divider-text { font-family:'Jost',sans-serif; font-size:11px; font-weight:300; letter-spacing:1px; text-transform:uppercase; color:#9C7B72; margin:0; }
  .glow-social {
    display:flex; align-items:center; justify-content:center; gap:9px;
    width:100%; padding:11px; border:1px solid rgba(200,137,110,.26); border-radius:11px;
    background:white; color:#3D1F14; font-family:'Jost',sans-serif;
  ...
  .glow-footer { text-align:center; margin-top:1.25rem; margin-bottom:0; font-family:'Jost',sans-serif; font-size:12px; color:#9C7B72; }
  .glow-alert {
    border-radius:11px; padding:12px 14px; margin-bottom:1rem;
  ...
  .glow-alert-info    { background:rgba(200,137,110,.12); border:1px solid rgba(200,137,110,.35); color:#7a4a30; }
  .glow-confirm-icon {
...
function Alert({ type = "error", children }) {
  return <div className={`glow-alert glow-alert-${type}`}>{children}</div>;
}
  ...
  return (
    <div>
      ...
      <p className="glow-confirm-body">
        ...
        <strong style={{ color: "#8B4D3A" }}>{email}</strong><br />
        The link expires in <strong>15 minutes</strong>.
        <br /><br />
        Didn't get it? Check your spam folder, or click below to resend.
      </p>
      <button className="glow-btn" onClick={onResend} disabled={loading}>
        {loading ? "Sending…" : "Resend Verification Email"}
      </button>
      <button className="glow-btn-ghost" onClick={onBackToLogin}>
        Back to Sign In
      </button>
    </div>
  );
      ...
      {alert && (
        <Alert type={alert.type}>
          ...
          {alert.action === "register" && (
            <> {" "}
              <a href="#" className="glow-link" style={{ fontWeight: 600 }}
                onClick={(e) => { e.preventDefault(); onSwitchWithEmail(alert.email); }}>
                Create an account →
              </a>
            </>
          )}
...
<a href="#" className="glow-link" style={{ fontSize: 11 }}
  ...
  Forgot password?
      ...
      <div style={{ marginBottom: "1rem", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        />
        <label htmlFor="remember" style={{ fontSize: 12, color: "#9C7B72", margin: 0 }}>
      ...
      <button type="button" className="glow-social" onClick={redirectToGoogle}>
...
function RegisterForm({ onSwitch, initialEmail = "" }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.terms) {
      return setAlert({ type: "error", message: "You must agree to the terms." });
    }
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem" }}>
        <div style={{ flex: 1 }}>
          ...
          <input
            type="text" className="glow-input" placeholder="Amara" required
        ...
        <div style={{ flex: 1 }}>
          ...
          <input
            type="text" className="glow-input" placeholder="Chen" required
      ...
      <div style={{ marginBottom: "1rem" }}>
        ...
        <div className="glow-input-wrap">
          <input
            type="email" className="glow-input" placeholder="you@example.com" required
            value={form.email}
            onChange={(e) => setForm({ ...form, email: e.target.value })}
          />
          <span className="glow-input-icon">✦</span>
      ...
      <div style={{ marginBottom: "1rem" }}>
        ...
        <div className="glow-input-wrap">
          <input
            type={showConfirm ? "text" : "password"} className="glow-input"
            placeholder="••••••••" required
      ...
      <div style={{ marginBottom: "1rem", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        />
        <label htmlFor="terms" style={{ fontSize: 12, color: "#9C7B72", margin: 0 }}>
          I agree to the{" "}
          <a href="#" className="glow-link">Terms of Service</a> and{" "}
...
export function VerifyEmailPage() {
 ...
 const hasRun = useRef(false);
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        ...
        if (status === 410) {
          const isResent = data?.message?.includes("new one has been sent");
          setStatus(isResent ? "resent" : "expired");
  ...
  const iconMap  = { loading: "⏳", success: "✅", expired: "⛔", resent: "📧", invalid: "❌" };
  const titleMap = {
    ...
    <>
      ...
      <div className="glow-wrapper">
        ...
        <div className="glow-card" style={{ textAlign: "center" }}>
          <div className="glow-confirm-icon" style={{ fontSize: 32 }}>
            {iconMap[status]}
          </div>
          <p className="glow-confirm-title">{titleMap[status]}</p>
          <p className="glow-confirm-body">{message || "Please wait…"}</p>

          {status === "success" && (
            <button className="glow-btn" onClick={() => navigate("/analyzer")}>
              Go to Dashboard
            </button>
          )}
...
/* ─────────────────────────────────────────────
   Root Export  (mounted at /auth)
───────────────────────────────────────────── */
export default function GlowAuth() {
  const [tab,          setTab]          = useState("login");
  const [prefillEmail, setPrefillEmail] = useState("");

  const switchToRegisterWithEmail = (email) => {
        ...
        <div className="glow-card">
          ...
          {tab === "login" ? (
            <LoginForm
              ...
              onSwitchWithEmail={switchToRegisterWithEmail}
            />
          ) : (
            <RegisterForm
