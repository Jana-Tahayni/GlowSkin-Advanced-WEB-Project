...
const HistoryPage = ({ onBack, authHeaders }) => {
  const [history,    setHistory]    = useState([]);
  const [loading,    setLoading]    = useState(true);
  const [search,     setSearch]     = useState("");
  const [filter,     setFilter]     = useState("");
  const [modalId,    setModalId]    = useState(null); // ← الـ modal
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const res  = await fetch(`/api/products/history?${params}`, { headers: authHeaders });
    ...
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "2.5rem 1.2rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          {history.map((item) => (
            <div
              ...
              key={item.id}
              onClick={() => setModalId(item.id)} // ← افتح الـ modal
            >
      ...
      {modalId && (
        <div
          ...
          onClick={() => setModalId(null)} // ← كليك برا يقفل
        >
          <div
            style={{
            ...
            onClick={(e) => e.stopPropagation()} // ← كليك جوا ما يقفل
          >
            <button
              className="btn-secondary"
