...
class CaseController extends Controller
{
    ...
    public function stats(): JsonResponse
    {
        return response()->json([
            ...
            'data'    => [
                ...
                'accuracy'    => round(Case_::whereNotNull('confidence')->avg('confidence')),
    ...
    {
        $case = Case_::with(['routines.steps', 'user'])->findOrFail($id);
    ...
    // POST /api/cases
    // بيتنادى لما المريض يدفع وتجي الحالة للدكتور
    public function store(Request $request): JsonResponse
    {
        ...
        // ── إرسال notification للدكتور لما تجي حالة جديدة ──
        $doctor = User::where('role', 'doctor')->first();
        if ($doctor) {
            $doctor->notify(new NewCaseReceived($case));
        }
